...
use crate::metrics::{new_counter_vec, new_histogram_vec, HistogramVec, IntCounterVec};
...
mod tests {
    ...
    impl PrometheusLabels<2> for HelloRequest {
        fn labels(&self) -> OwnedPrometheusLabels<2> {
            OwnedPrometheusLabels {
                labels: [Cow::Borrowed("hello"), Cow::Owned(self.name.clone())],
            }
        }
    }

    impl PrometheusLabels<2> for GoodbyeRequest {
        fn labels(&self) -> OwnedPrometheusLabels<2> {
            OwnedPrometheusLabels {
                labels: [Cow::Borrowed("goodbye"), Cow::Owned(self.name.clone())],
            }
        }
    }
